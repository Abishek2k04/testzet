...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    isGlobalDebuggingEnabled: function() {
        if(JotForm.hasAccessToLocalStorage() && JotForm.initializing === false) {
            return localStorage.getItem('__jfConditionDebugger.isGlobalDebuggingEnabled') === 'true';
        }
    ...
    checkCondition: function (condition, sourceField, sourceEvent) {
        ...
        if (JotForm.isGlobalDebuggingEnabled()) {
            debugger;
        }
                    ...
                    try {
                        ...
                        if (equation.charAt(i + 1) === '(' || (equation.charAt(i + 1) === '[' && equation.charAt(i + 2) === '(')) {
                            ...
                            if (specOp === 'dateString') {
                                ...
                                var date = new Date(args[0] * 24 * 60 * 60 * 1000);
    ...
    */
    ...
    getSubscriptionPriceSummary: function() {
        ...
        const first_discounted = (firstPaymentDiscount && (first - firstPaymentDiscount) > 0) ? (first - firstPaymentDiscount) : 0;
        const recurring_discounted = (recurPaymentDiscount && (recurring - recurPaymentDiscount) > 0) ? (recurring - recurPaymentDiscount) : 0;
            ...
            var handleFormSubmit = function (e) {
                ...
                if (
                    ...
                    if (JotForm.newPaymentUI) {
                        ...
                        if (window.paymentType === 'subscription') {
                            ...
                            form.insert(new Element('input', {
                                type: 'hidden',
                                name: 'paymentSummary'
                            }).putValue(JSON.stringify(JotForm.getSubscriptionPriceSummary())));
                        }
                    ...
                    if (JotForm.pricingInformations && JotForm.pricingInformations.general) {
                        ...
                        const taxDiscountValue = (taxTotalWithoutDiscount - tax_total) || 0;
                        const shippingDiscountValue = shippingDiscountVal || 0;
                        const shippingDiscountApplied = JotForm.discounts.shipping;

                        const paymentSummary = {
                            shipping: (shippingDiscountApplied ? oldShippingTotal : shipping) || 0,
                            shipping_discounted: (shippingDiscountApplied ? shipping : 0) || 0,
                            tax: taxTotalWithoutDiscount || 0,
                            tax_discounted: (taxTotalWithoutDiscount === tax_total ? 0 : tax_total) || 0,
                            subtotal: item_total || 0,
                            total: net_amount || 0,
                            discount: (shippingDiscountValue + taxDiscountValue + discount) || 0
                        };
                        form.insert(new Element('input', {
                            type: 'hidden',
                            name: 'paymentSummary'
